...
USER_NAME = os.environ['USER_NAME'] # 'alfaarizi'
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    justify_format(root, 'last_updated', datetime.datetime.now().strftime("%Y-%m-%d %H:%M UTC"), 22)
...
if __name__ == '__main__':
    """
    Andrew Grant (alfaarizi), 2022-2025
    """
    print('Calculation times:')
    # define global variable for owner ID and calculate user's creation date
    # e.g {'id': 'MDQ6VXNlcjU3MzMxMTM0'} and 2019-11-03T21:15:07Z for username 'alfaarizi'
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    formatter('account data', user_time)
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2004, 5, 1))
    ...
    if OWNER_ID == {'id': 'U_kgDOB0TvTA'}: # only calculate for user alfaarizi
